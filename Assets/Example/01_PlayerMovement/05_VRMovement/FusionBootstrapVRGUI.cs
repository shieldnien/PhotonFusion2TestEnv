namespace Example.VRMovement
{
	...
	public sealed class FusionBootstrapVRGUI : Fusion.Behaviour
	{
		[SerializeField]
		...
		{
			SetVisibility(false);
			GetComponent<FusionBootstrap>().StartSinglePlayer();
		}

		public void StartSharedClient()
		{
			SetVisibility(false);
			GetComponent<FusionBootstrap>().StartSharedClient();
		}

		public void StartHost()
		{
			SetVisibility(false);
			GetComponent<FusionBootstrap>().StartHost();
		}

		public void StartServer()
		{
			SetVisibility(false);
			GetComponent<FusionBootstrap>().StartServer();
		}

		public void StartClient()
		{
			SetVisibility(false);
			GetComponent<FusionBootstrap>().StartClient();
		}

		public void StartAutoHostOrClient()
		{
			SetVisibility(false);
			GetComponent<FusionBootstrap>().StartAutoClient();
		}
		...
		private void Awake()
		{
			...
			GetComponent<FusionBootstrap>().enabled = isVREnabled == false;
		}
