namespace Example.LagCompensation
{
	...
	/// </summary>
	public sealed class LagCompensationFirstPersonPlayer : FirstPersonExpertPlayer
	{
		const int MAX_RECORDS = 100;

		[SerializeField]
		private float _raycastLength = 30.0f;
		...
		protected override void ProcessInputSortedByLocalAlpha()
		{
			...
			if (Input.WasActivated(EExpertInputAction.LMB) == true && Runner.IsForward == true)
			{
				...
				if (KCC.ResolveRenderPositionAndLookRotation(CameraHandle, Input.FixedInput.LocalAlpha, out Vector3 renderCameraPosition, out Quaternion renderLookRotation) == true)
				{
					FireAccurate(renderCameraPosition, renderLookRotation);
				}
				else
				{
		...
		protected override void ProcessInputAfterRenderMovement()
		{
			...
			if (Input.WasActivated(EExpertInputAction.LMB) == true)
			{
				...
				if (KCC.ResolveRenderPositionAndLookRotation(CameraHandle, Input.RenderInput.LocalAlpha, out Vector3 renderCameraPosition, out Quaternion renderLookRotation) == true)
				{
					FireRender(renderCameraPosition, renderLookRotation, _renderKCCRecords, ref _renderKCCCount);
				}
				else
				{
					KCC.LogError("Missing data for lag calculation of render position and rotation.");
