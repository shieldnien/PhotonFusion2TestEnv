namespace Example
{
	...
	public sealed class SortedUpdateInvoker : SimulationBehaviour, IDespawned
	{
		...
		public void Invalidate(ISortedUpdate callback)
		{
			for (int i = 0, count = _scheduledObjects.Count; i < count; ++i)
			{
				CallbackObject scheduledObject = _scheduledObjects[i];
				if (ReferenceEquals(scheduledObject.Callback, callback) == true)
				{
					int targetIndex = _scheduledObjects.Count - 1;

					_scheduledObjects[i] = _scheduledObjects[targetIndex];
					_scheduledObjects.RemoveAt(targetIndex);

					scheduledObject.Clear();
					_cache.Push(scheduledObject);
